...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # Iterate over old positions and update new probability table
        # in following way 
        # P(NEW = pos_n) = 
        # sum over all pos_prev {P(PREV = pos_prev) * P(NEW = pos_n | PREV = pos_prev)}
        for pos_prev in positions:
            stateWithGhostSet = self.setGhostPosition(gameState, pos_prev)
            givenPrevDist = self.getPositionDistribution(stateWithGhostSet)
            for pos_n in givenPrevDist:
                prob_of_pos_n_given_prev = givenPrevDist[pos_n]
                prob_of_pos_prev = self.beliefs[pos_prev]
                newPosDistribution[pos_n] += prob_of_pos_prev * prob_of_pos_n_given_prev

        # Update beliefs to new distribution
        self.beliefs = newPosDistribution
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for pos in cycledList(legalPos, self.numParticles):
            uniformDist.append(pos)

        self.particles = uniformDist
    ...
    def observe(self, observation, gameState):
        ...
        # Check if ghost is captured, then distribution is trivial
        if noisyDistance == None:
            self.particles = [ self.getJailPosition() for i in range(self.numParticles) ]
            return 

        # Compute posterior distribution given noisy data
        posteriorDist = util.Counter()
        priorDist = self.getBeliefDistribution()
        ...
        isNonZero = False
        for elem in posteriorDist:
            if posteriorDist[elem] != 0:
                isNonZero = True
                break
        if not isNonZero:
            self.initializeUniformly(gameState)
        ...
        self.particles = [util.sample(posteriorDist) for i in range (self.numParticles)]
        

            
    def getDist(self, gameState, prevPos):
        return self.getPositionDistribution(self.setGhostPosition(gameState, prevPos))
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        transform = lambda p: util.sample(self.getDist(gameState, p))
        newParticles = map(transform, self.particles)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        cartesianProd = itertools.product
        positions = list(cartesianProd(self.legalPositions, repeat=self.numGhosts))
        ...
        self.particles = []
        for p in cycledList(positions, self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                posterior = getPositionDistributionForGhost(
                                                   setGhostPositions(gameState, newParticle),
                                                   i, self.ghostAgents[i]
                                                            )
                newParticle[i] = util.sample(posterior)
            "*** END YOUR CODE HERE ***"
